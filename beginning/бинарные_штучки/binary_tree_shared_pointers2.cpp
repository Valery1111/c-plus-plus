...
public:
    ...
    std::shared_ptr<A> rightChild; // указатель на правый узел-потомок
...
std::shared_ptr<A> insert(std::shared_ptr<A> node, int key)
{
    ...
    if (node == nullptr)
    {
        // TODO: создать экземпляр класса A, в конструктор передать key
        // TODO: создать shared_ptr к созданному экземпляру
        // TODO: вернуть этот shared_ptr

        std::shared_ptr<A> cell(new A(key));
        return cell;
    }
